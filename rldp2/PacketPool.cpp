...
thread_local BufferSlicePool::ThreadLocalPool* g_buffer_pool = nullptr;

BufferSlicePool::ThreadLocalPool& BufferSlicePool::get_thread_pool() {
  if (!g_buffer_pool) {
    g_buffer_pool = new ThreadLocalPool();
    g_buffer_pool->cached_buffers.reserve(kMaxCachedBuffers);
  }
...
BufferSlicePool::Stats BufferSlicePool::get_stats() {
  ...
  return g_buffer_pool->stats;
}

void BufferSlicePool::reset_stats() {
  if (g_buffer_pool) {
    g_buffer_pool->stats = Stats{};
    g_buffer_pool->stats.cached_buffers = g_buffer_pool->cached_buffers.size();
  }
}
