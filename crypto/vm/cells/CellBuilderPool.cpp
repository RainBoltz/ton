...
thread_local CellBuilderPool::ThreadLocalPool* g_pool = nullptr;

CellBuilderPool::ThreadLocalPool& CellBuilderPool::get_thread_pool() {
  if (!g_pool) {
    g_pool = new ThreadLocalPool();
    // Pre-allocate initial chunk to avoid allocations during first operations
    g_pool->free_list.reserve(kMaxFreeList);
  }
  return *g_pool;
}
...
void CellBuilderPool::reset_stats() {
  if (g_pool) {
    g_pool->stats = Stats{};
    g_pool->stats.pool_size = g_pool->free_list.size();
  }
}
